...
const COOKIE_FILE = process.env.COOKIE_FILE || paths.cookieFile();
const QQ_COOKIE_FILE = process.env.QQ_COOKIE_FILE || paths.qqCookieFile();
const UPDATE_WORK_DIR = process.env.MINERADIO_UPDATE_DIR || paths.updateWorkDir();
const UPDATE_DOWNLOAD_DIR = process.env.MINERADIO_UPDATE_DOWNLOAD_DIR || paths.updateDownloadDir();
const UPDATE_PATCH_BACKUP_DIR = process.env.MINERADIO_PATCH_BACKUP_DIR || paths.updatePatchBackupDir();
const BEATMAP_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR || paths.getBeatMapCacheDir();
...
function beatCacheRootInfo() {
  ...
  const allowed = isWin ? (!!root && !/^C:$/i.test(drive)) : true;
  const available = isWin ? (allowed && fs.existsSync(root)) : true;
